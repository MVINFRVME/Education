...
def passport_info(nums):
    for i_num, i_name in data.items():
        if i_num == nums:
            return ' '.join(i_name)


my_serial = int(input('Введите серию паспорта: '))
my_number = int(input('Введите номер паспорта: '))
serial_number = my_serial, my_number
print(passport_info(serial_number))
